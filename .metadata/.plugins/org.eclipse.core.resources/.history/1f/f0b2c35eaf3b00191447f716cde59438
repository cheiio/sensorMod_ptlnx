...
int main(int argc, char *argv[]) {
	...
	// --------------------------------------------- LED write
    xil_uio__write32(gpioLED_handler, XIL_GPIO_1_TRI_OFFSET, 0x00); // set as output
    ...
    sensorMod__set_butter(sensor_handler, ab);
    sensorMod__set_centerCurr(sensor_handler, cenCurr);
    sensorMod__reset(sensor_handler);
    sensorMod__printConfig(sensor_handler);

    printf("\n\t %d , %d\n", sensor_handler->lastTime, millis());
    //float Q[3];
    ...
    for (uint32_t i=0; i<n_data; i++){
		...
		// flag timing
		xil_uio__write32(gpioLED_handler, XIL_GPIO_1_OFFSET, 0xFF);
    	...
    	// flag timing
		xil_uio__write32(gpioLED_handler, XIL_GPIO_1_OFFSET, 0);
